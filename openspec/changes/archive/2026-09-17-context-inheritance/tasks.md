...
- [x] 1.1 Red — direct unit tests `test/context/normalize-context.test.ts`: string →
...
- [x] 1.2 Red — direct unit tests `test/context/merge-contexts.test.ts`: four-level precedence
      ...
      for `{ notes: ["x"] }`.
- [x] 1.3 Green — `src/context/{normalize-context,merge-contexts,is-empty-context,reserved-context-keys}.ts`,
...
- [x] 2.1 Red — `test/api/context.test.ts` part 1: `context` `Context forms and normalization`
...
- [x] 2.2 Green — `SemanticRuleOptions.context` + validation/copy in `rules/normalize-rule.ts`;
...
- [x] 3.1 Red — `test/api/context.test.ts` part 2: `Deterministic merge` scenarios (inherits
...
- [x] 3.2 Green — `schema/is-reserved-path.ts` wired into rule and node path validation
...
- [x] 4.1 Red — `test/api/context-groups.test.ts`: `compilation` MODIFIED scenarios "Uniform
...
- [x] 4.2 Green — `compiler/plan-groups.ts` (bound rules + surviving set → ordered groups by
...
- [x] 5.1 Red — `test/api/context-snapshot.test.ts`: `Context in state` scenarios: "Context key
...
- [x] 5.2 Green — review and commit the new snapshot; confirm `git diff` shows no change to the
      bootstrap snapshot file.

## 6. Type tests

- [x] 6.1 Red — `test/types/context.test-d.ts`: `Context` accepted at `createEDcheck`, `define`
      ...
      `semantic("x", { context })`).
- [x] 6.2 Green — adjust `NodePath<T>`, `ContextObject` index signature and option types until
      `yarn typecheck` and `vitest --typecheck` pass.

## 7. Surface, docs and roadmap

- [x] 7.1 Assert `test/api/public-surface.test.ts` is unchanged and still green (no runtime
      symbol added). Extend `test/providers/import-boundaries.test.ts` with a `src/context/**`
      check: only `src/shared` imports allowed.
- [x] 7.2 README: "Context" section — string vs object, reserved keys, the four levels with one
...
- [x] 7.3 `openspec/roadmap.md`: change 2 status; run `openspec validate --all` and `yarn verify`.
