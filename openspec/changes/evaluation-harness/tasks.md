...
- [ ] 1.1 Red — `test/helpers/fixtures/fixture-file-schema.test.ts` with synthetic files:
...
- [ ] 1.2 Green — `test/helpers/fixtures/{fixture-file-schema,load-fixture}.ts`,
      `types/{fixture-case,fixture-file,fixture-expect,fixture-binding}.ts`.

## 2. Fixture corpus and coverage test

- [ ] 2.1 Red — `test/fixtures/fixtures.test.ts` with `Fixture format and coverage` scenarios:
...
- [ ] 2.2 Green — migrate `full-name`, `age-occupation`, `project-description` to the D1
...
- [ ] 3.1 Red — `test/api/fixture-payloads.test.ts` with `Fixture registry and payload
...
- [ ] 3.2 Green — `test/fixtures/{full-name,age-occupation,bio-consistency,project-description}/binding.ts`
...
- [ ] 4.1 Red — extend `fixtures.test.ts` coverage (automatic) and `fixture-payloads.test.ts`
      with "Context binding state".
- [ ] 4.2 Green — `test/fixtures/project-name/{es,en}.json` and `binding.ts` with the schema
      context from design D2; add to `registry.ts`.

## 5. Eval helpers (offline)

- [ ] 5.1 Red — `test/helpers/eval/has-eval-key.test.ts` ("hasEvalKey");
...
- [ ] 5.2 Green — `test/helpers/eval/{has-eval-key,describe-eval,resolve-eval-provider,run-cases,count-misses,format-miss,write-baseline}.ts`,
...
- [ ] 6.1 Red — `test/helpers/calibration/derive-thresholds.test.ts` with `Baseline and
...
- [ ] 6.2 Green — `test/helpers/calibration/derive-thresholds.ts`, `types/calibration.ts`.

## 7. Eval suites, baseline and the calibrated constant

- [ ] 7.1 Red — `test/eval/noul-rules.eval.test.ts` ("One test per rule and language" is
...
- [ ] 7.2 Green — implement the two eval suites; add `"eval"` script to `package.json`. Run
...
- [ ] 8.1 Red — `outcome-policy` "Provenance is documented": a test reads `docs/calibration.md`
      and asserts it contains the current `pass`/`fail` values, `baseline.recordedAt` and
      `baseline.model`.
- [ ] 8.2 Green — `docs/calibration.md` (method, current values, provenance, recalibration
