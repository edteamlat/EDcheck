...
- [x] 1.1 Red — extend `test/package-contract.test.ts` with `Optional peer package contract`
...
- [x] 1.2 Green — `yarn add -D ai@^7.0.105 @ai-sdk/gateway@^4.0.85`; add both to
...
- [x] 2.1 Red — `test/errors/edcheck-errors.test.ts`: `EDcheckProviderError("sdk", { cause })`
...
- [x] 2.2 Green — extend the code unions in `src/errors/`; add
...
- [x] 3.1 Red — `test/providers/gateway-missing-peer.test.ts` with `Gateway adapter construction
and lazy dependency` scenarios: "Construction is synchronous and does not import the SDK",
      ...
      `index.ts`).
- [x] 3.2 Green — `src/providers/gateway/load-ai-sdk.ts` (memoized `import("ai")` /
...
- [x] 4.1 Red — `test/providers/gateway-provider-http.test.ts`: "String model uses the
...
- [x] 4.2 Green — `src/providers/gateway/resolve-evaluation-model.ts`: string →
...
- [x] 5.1 Red — `test/providers/gateway-provider.test.ts` with `Gateway request and response
...
- [x] 5.2 Green — `src/providers/gateway/to-gateway-questions.ts`,
...
- [x] 6.1 Red — extend `test/providers/gateway-provider.test.ts` with `Gateway error mapping`
...
- [x] 6.2 Green — `src/providers/gateway/map-sdk-error.ts` implementing the D5 table using the
...
- [x] 7.1 Red — `test/providers/gateway-score.test.ts` with `Gateway Score normalization`
...
- [x] 7.2 Green — extend `to-gateway-questions.ts` (passthrough) and `from-gateway-answers.ts`
...
- [x] 8.1 Red — `test/api/gateway-policy.test.ts` with `Gateway policy parity` scenarios:
...
- [x] 8.2 Green — expected to pass without new code; fix any adapter divergence found (for
      example abort reason propagation through the SDK).

## 9. Key detection

- [x] 9.1 Red — `test/providers/provider-from-env.test.ts` with `Provider selection from
environment` scenarios: "TypeSafe key only", "Gateway key only", "Both keys default to
      ...
      "Invalid prefer".
- [x] 9.2 Green — `src/providers/provider-from-env.ts` and its `types/`. Trim before the empty
      check. Validate `prefer` against the union (`invalid_option`).

## 10. Public surface, eval and docs

- [x] 10.1 Red — update the sorted symbol list in `test/api/public-surface.test.ts` with
...
- [x] 10.2 Green — export both from `src/index.ts` (plus `GatewayProviderOptions`,
