...
- [ ] 1.1 Add Vitest alias `edcheck → ./src/index.ts` in `vitest.config.ts` and the matching
...
- [ ] 1.2 Extend `test/package-contract.test.ts` with the `provider` scenario "Package pulls in
      neither ai nor the SDK". Passes immediately; it guards D3 from now on.

## 2. Errors and shared utilities

- [ ] 2.1 Red — direct unit tests `test/shared/paths.test.ts`: `parsePath("a.b.c")` →
...
- [ ] 2.2 Green — `src/shared/{parse-path,format-path,is-path-prefix,get-at-path,set-at-path}.ts` + `src/shared/index.ts`.
- [ ] 2.3 Red — direct unit tests `test/shared/combine-signals.test.ts`: aborting either input
...
- [ ] 2.4 Green — `src/shared/{combine-signals,is-browser-environment,assert-server-environment}.ts`,
...
- [ ] 3.1 Red — `test/api/semantic-rule.test.ts` covering all `semantic-rules` scenarios except
...
- [ ] 3.2 Green — `src/rules/{semantic,normalize-rule}.ts`, `src/rules/types/{semantic-rule,semantic-rule-options,severity}.ts`,
      `src/policy/types/thresholds.ts`, `src/policy/validate-thresholds.ts`. Export `semantic`
      and the types.

## 4. Outcome policy (pure logic)

- [ ] 4.1 Red — direct unit tests `test/policy/map-probability-to-outcome.test.ts`: `0.95`,
...
- [ ] 4.2 Green — `src/policy/{default-thresholds,resolve-thresholds,map-probability-to-outcome,resolve-severity}.ts`,
      `src/policy/types/{outcome,failure-policy}.ts`. Export `DEFAULT_THRESHOLDS`.

## 5. Provider contract and mock adapter

- [ ] 5.1 Red — `test/providers/mock-provider.test.ts`: default answer, answers by id, answers by
...
- [ ] 5.2 Green — `src/providers/types/{semantic-provider,semantic-request,semantic-response,semantic-question,semantic-answer,semantic-usage}.ts`,
...
- [ ] 5.3 Red — `test/providers/import-boundaries.test.ts`: read every file under
...
- [ ] 6.1 Red — `test/api/create-edcheck.test.ts`: `Instance creation` scenarios (defaults,
...
- [ ] 6.2 Red — `test/api/define.test.ts`: `Binding rules to leaf paths` scenarios: top-level,
      ...
      only).
- [ ] 6.3 Green — `src/schema/{unwrap-node,classify-node,resolve-node,collect-invalid-prefixes}.ts`,
...
- [ ] 7.1 Red — `test/fixtures/full-name/{es,en}.json` with `positive`, `negative`, `ambiguous`
...
- [ ] 7.2 Green — `src/compiler/{question-template,build-question,build-state,compile-request}.ts`,
...
- [ ] 8.1 Red — extend `test/api/safe-parse.test.ts` with `schema-binding` `Shape-first
...
issues pass through unchanged` scenarios, `Success semantics` scenarios ("Mixed Zod and
      ...
      rules".
- [ ] 8.2 Green — invalid-prefix collection, node re-parse in the failure branch, `undefined`/
...
- [ ] 9.1 Red — `test/api/outcome-policy.test.ts`: `Probability to outcome mapping` scenarios
...
- [ ] 9.2 Red — `test/api/values.test.ts`: `compilation` `User values never enter the question`
      scenarios: adversarial value, template-like characters, empty and whitespace-only, 50 000
      characters, emoji and RTL.
- [ ] 9.3 Green — wire thresholds/severity precedence into `run-safe-parse.ts`; fix anything the
...
- [ ] 10.1 Red — `test/api/failure-policy.test.ts`: `outcome-policy` `Failure policy` scenarios:
...
- [ ] 10.2 Red — `test/api/cancellation.test.ts`: `schema-binding` `Cancellation and timeout`
...
- [ ] 10.3 Green — failure policy branch, response validation (every id present, `noul ∈ [0,1]`),
      `combineSignals` usage, abort vs timeout discrimination in `run-safe-parse.ts`.

## 11. TypeSafe adapter

- [ ] 11.1 Red — `test/providers/typesafe-provider.test.ts` with an injected `fetch` recording
...
- [ ] 11.2 Green — `src/providers/typesafe/{typesafe-provider,response-schema,map-response,should-retry}.ts`,
...
- [ ] 11.3 Red then green — `test/api/custom-provider.test.ts`: `provider` "Custom provider is
      accepted by createEDcheck".

## 12. Type tests

- [ ] 12.1 Red — `test/types/semantic-result.test-d.ts`: `result` `Type preservation` scenarios
      ...
      `FieldPath` depth.
- [ ] 12.2 Green — adjust `FieldPath<T>` and public type exports until `yarn typecheck` and
      `vitest --typecheck` pass.

## 13. Smoke eval against real Jev

- [ ] 13.1 `test/eval/full-name.eval.test.ts`: `describe.skipIf(!process.env.TYPESAFE_API_KEY)`;
...
- [ ] 14.1 Red — `test/api/public-surface.test.ts`: sorted `Object.keys(await import("edcheck"))`
...
- [ ] 14.2 Green — prune or add exports in `src/index.ts` until the list matches exactly.
- [ ] 14.3 README: install (`yarn add edcheck zod`), shared Zod schema on the client, server route
...
- [ ] 14.4 Docs: add the `api/` row to constitution §11 and the `api → …` step to the flow line;
