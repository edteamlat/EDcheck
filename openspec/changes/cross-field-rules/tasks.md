...
- [ ] 1.1 Red — direct unit tests `test/shared/format-path-list.test.ts`: one path → "\`a\`";
...
- [ ] 1.2 Green — `src/shared/format-path-list.ts`, `src/rules/{is-path-like-token,extract-backtick-references}.ts`.

## 2. Binding validation

- [ ] 2.1 Red — `test/api/cross-field-define.test.ts`: `cross-field-rules` `Cross-field binding`
...
- [ ] 2.2 Green — `src/api/types/{cross-field-binding,node-path}.ts` (create `node-path` only if
...
- [ ] 3.1 Red — `test/fixtures/age-occupation/{es,en}.json` with `positive`, `negative`,
...
- [ ] 3.2 Green — `src/compiler/{cross-field-question-template,build-cross-field-question}.ts`;
...
- [ ] 4.1 Red — extend `test/api/cross-field-parse.test.ts`: "Shape failure on a declared path
...
- [ ] 4.2 Green — per-declared-path exclusion (prefix check against every declared path), node
      re-parse for each declared node in the failure branch, nullish skip in
      `api/run-safe-parse.ts`.

## 5. Attribution in the result

- [ ] 5.1 Red — `test/api/cross-field-result.test.ts`: `result` MODIFIED "Field-rule issue has no
      ...
      false once).
- [ ] 5.2 Green — `Issue.paths?` in `src/result/types/issue.ts`; `semantic-issue` and
...
- [ ] 6.1 Red — `test/types/cross-field.test-d.ts`: `Type-level contract` scenarios (valid tuple,
      unknown path, array path, empty tuple, `crossField` optional) and `Issue["paths"]` is
      `(string | number)[][] | undefined`.
- [ ] 6.2 Green — adjust `CrossFieldBinding<T>` / `NodePath<T>` until `yarn typecheck` and
      `vitest --typecheck` pass.

## 7. Eval, surface, docs, roadmap

- [ ] 7.1 `test/eval/age-occupation.eval.test.ts`: `describe.skipIf(!process.env.TYPESAFE_API_KEY)`;
      positives `≥ 0.6` (or no issue), negatives `≤ 0.4`, ambiguous listed only. Run once locally
      with a key.
- [ ] 7.2 Assert `test/api/public-surface.test.ts` is unchanged and green (no runtime symbol).
- [ ] 7.3 README: "Cross-field rules" section — binding example (PDR age/occupation), backtick
...
- [ ] 7.4 `openspec/roadmap.md`: change 3 status. `openspec validate --all` and `yarn verify`.
