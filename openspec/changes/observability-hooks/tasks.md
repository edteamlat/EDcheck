...
- [ ] 1.1 Red — `test/types/observability.test-d.ts` with `Observability type contract`
...
- [ ] 1.2 Green — `src/api/types/{edcheck-hooks,provider-event-base,provider-request-event,provider-response-event,provider-error-event,provider-error-kind}.ts`;
...
- [ ] 2.1 Red — `test/shared/create-id.test.ts`: returns a UUID v4-shaped string; 1 000 calls
      ...
      passed by reference.
- [ ] 2.2 Green — `src/shared/create-id.ts` (`globalThis.crypto.randomUUID()`),
      `src/api/invoke-hook.ts`.

## 3. Request and response events

- [ ] 3.1 Red — extend `test/api/observability.test.ts` with `Request events` scenarios: "One
...
- [ ] 3.2 Green — in the request-execution step: generate `parseId` once per parse and
...
- [ ] 4.1 Red — extend with `Error events` scenarios: "Provider error", "Timeout", "Malformed
...
- [ ] 4.2 Green — on rejection classify: caller signal aborted → build the `EDcheckAbortError`
...
- [ ] 5.1 Red — extend with `Zero-request parses fire nothing` scenarios: "Root shape failure",
      ...
      swallow the abort".
- [ ] 5.2 Green — expected to pass through `invoke-hook`; fix any emission placed before the
      zero-request early return.

## 6. No output

- [ ] 6.1 Red — `test/api/no-output.test.ts` with `No output by default` "Silent across
...
- [ ] 6.2 Green — expected to pass; remove any stray output if found.

## 7. Multi-group and node parses (conditional)

- [ ] 7.1 Red — extend with `Multi-group and node parses` scenarios: "Two groups, two request
...
- [ ] 7.2 Green — `run-rules` (or the group loop) stamps `requestIndex`/`requestCount` per group
      and receives `entry`/`path` from its caller; node parse passes `entry: "node"` and its path.

## 8. Docs and roadmap

- [ ] 8.1 Red — none.
- [ ] 8.2 Green — README section "Observability": event fields table, OpenTelemetry-style span
