...
- [x] 1.1 Red — none. The existing `test/api/**` suite and payload snapshots are the guard; they
      must stay green and unmodified through 1.2.
- [x] 1.2 Green — extract `src/api/run-rules.ts` (steps 5–8 of bootstrap D6: `plan-groups`,
...
- [x] 2.1 Red — `test/types/node-validation.test-d.ts` with `Node type contract` scenarios: "Leaf
...
- [x] 2.2 Green — `src/api/types/path-value.ts`, `src/api/types/semantic-node.ts`;
...
- [x] 3.1 Red — `test/api/node-validation.test.ts` with `Node handle construction` scenarios:
...
- [x] 3.2 Green — `src/schema/select-rules-under-path.ts` (field rules by `isPathPrefix`;
...
- [x] 4.1 Red — extend `test/api/node-validation.test.ts` with `Node pipeline` scenarios: "Shape
...
- [x] 4.2 Green — `src/result/prefix-issue-paths.ts`, `src/api/run-node-parse.ts` (D4 steps
      1–4, then `run-rules`). Wire `SemanticNode.safeParse`. Reuse `collect-invalid-prefixes`
      and `get-at-path`; no new path helpers.

## 5. Context preservation

- [x] 5.1 Red — extend the suite with `Context preservation in node parses` scenarios: "Instance
      ...
      adds no key".
- [x] 5.2 Green — expected to pass with no new code because `run-rules` consumes the effective
      context stored at `define`; fix any leak found.

## 6. Cross-field selection (only when `cross-field-rules` is applied)

- [x] 6.1 Red — extend the suite with `Rule selection for a node` cross-field scenarios:
...
- [x] 6.2 Green — complete the cross-field branch of `select-rules-under-path.ts`; values for a
      covered cross-field rule are read relative to the node for each declared path.

## 7. Cancellation and concurrency

- [x] 7.1 Red — extend the suite with `Node cancellation and concurrency` scenarios:
...
- [x] 7.2 Green — expected to pass through `run-rules`; if the late-response case leaks, fix the
...
- [x] 8.1 Red — none.
- [x] 8.2 Green — README section "Validate one field": on-blur example with `AbortController`
