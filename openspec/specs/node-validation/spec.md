...
Expose `SemanticSchema.node(path)` so a bound leaf or object can be validated independently, selecting only the rules under that path while preserving whole-object compilation, context, outcomes, and cancellation.
